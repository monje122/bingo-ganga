...
const supabase = window.supabase.createClient(supabaseUrl, 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9qdWNteXNudWRnb3puZGV5aHZ6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTI5NDE4NDksImV4cCI6MjA2ODUxNzg0OX0.bUoYtdNIyzpSMLsIpA9fTokyEg8yv3AzPeo5S9eK8ro');
...
async function consultarCartones() {
  ...
  data.forEach(item => {
    item.cartones.forEach(num => {
      ...
      img.src = `${supabaseUrl}/storage/v1/object/public/cartones/SERIAL_BINGOGANGA_CARTON_${String(num).padStart(5, '0')}.jpg`;
...
function abrirModalCarton(numero, elemento) {
  ...
  img.src = `${supabaseUrl}/storage/v1/object/public/cartones/SERIAL_BINGOGANGA_CARTON_${String(numero).padStart(5, '0')}.jpg`;
